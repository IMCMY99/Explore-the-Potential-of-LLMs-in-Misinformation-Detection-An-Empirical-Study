...
client = OpenAI(
    # 输入转发API Key
    api_key="sk-YfDUjMHw0WDvM3DcJkWM9ED7snCTk07Cmvj7hHsbewDgsPTC",
...
prompt_1_0 = f"Given the following news [claim_text] ,We need you to detect fake news. Here are the options for categorizing the underlying causes of fake news: \
    [A. Emotional bias or misleading intent,\
    B. Lack of evidence or credible sources,\
    C. Conflicting fact,\
    D. Informal statements, expressions, or vague language,\
    E. insufficient supporting materials,\
    F. Lack of context or taken out of context,\
    G. Misinterpretation or misquotation,\
    H. Oversimplification or exaggeration,\
    I. Doctored images or videos,\
    J. Other reasons]\
...
prompt_2_0 = f"Now you are an annotator to determine whether a given news is fake news. The news is given in the form that 'Is [News] a true news? Answer: [Answer]'. Here are the options for categorizing the underlying causes of fake news: \
    [A. Emotional bias or misleading intent,\
    B. Lack of evidence or credible sources,\
    C. Conflicting fact,\
    D. Informal statements, expressions, or vague language,\
 ...
 You need to give an answer in the [Answer] slot. There are two available answers that you can choose to fill the slot: 0 and 1,\
  if the news is true, you are supposed to answer 1, otherwise you should answer 0. You have to choose one of the two answers from 0 and 1. No need to answer any other words expect 0 and 1. "

prompt_3_0 = f"Now we need you to detect fake news. Here are the options for categorizing the underlying causes of fake news: \
    [A. Emotional bias or misleading intent,\
    B. Lack of evidence or credible sources,\
    C. Conflicting fact,\
    D. Informal statements, expressions, or vague language,\
    ...
    I. Doctored images or videos,\
    J. Other reasons]\
     Below, I will provide you with a news. If the news is true, please reply with 1.If the news is fake, please reply with 0. Do not reply any other words except 1 and 0."
# 遍历数据并进行评估
# Data = Data[4475:]
...
Prompts = [prompt_1_0, prompt_2_0, prompt_3_0]
for i in range(3):
    pred = []
    if i < 1:
        ...
        with open("RA_prompt" + str(i) + ".txt", "w", encoding='utf-8') as file:
            for n, claim in tqdm(enumerate(claims), total=len(claims)):
